...
#### Add
Somente arquivos *staged* podem ser commitados, e para isso é precisso escolher *untrackeds* ou *not stageds*. Com ``git add <file>`` ele troca de estado para *staged* e com ``git reset -- <file>``.
Dicas:
...
- Utilizar verbos no imperativo sobre o que aqeuele commit afeta no código. Exemplo: ao invés de escrever "Eu adicionei teste X" ou "Adicionando teste X", use "Adiciona teste X".
...
### Gitflow
[Gitflow](https://danielkummer.github.io/git-flow-cheatsheet/index.pt_BR.html) é uma abstração de comandos do git juntamente com uma metodologia de versionamento de código. Com ele existirão novos comandos e convenções.
A primeira das convenções é a existência de duas branches principais no remote, sendo a `master`e a `develop`. Sendo a primeira modificada somente através de merges com a develop.
