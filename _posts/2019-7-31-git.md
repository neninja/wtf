...
### push
- **Estrutura**: ``git push <remoto> <branch>``
    - `<remoto>` nome do remoto, normalmente *origin* ou *upstream*
    - `<branch>` branch do remoto
- **Descrição**: Para enviar o código juntamente com seu histórico para o Github, deve-se utilizar o comando ``git push <nome-do-remote> <nome-da-branch>``. Caso ainda não esteja trabalhando com branches, utilize `master` no nome da branch. Mais a frente o conceito será abordado.
...
- **Estrutura**: ``git checkout <referência>``
    - ``<referência>`` branch ou hash do commit
- **Descrição**: Troca de branch ou acessa commit transformando-o em uma branch temporária - ideal para inspecionar estado do projeto naquele ponto
...
- **Descrição**: É possível retornar o histórico ou alterar a area de *staged*. Todos arquivos dos commits apagados permanecem (como `modified` ou `untracked`).
- **Flags**:
...
- **Estrutura**: ``git pull <remoto> <branch>``
    - `<remoto>` nome do remoto, normalmente *origin* ou *upstream*
    - `<branch>` branch do remoto
- **Descrição**: Caso tenha feito ``git clone`` ou ``git push`` em algum momento, você está apto para atualizar seu código de acordo com o seu `remote`.
